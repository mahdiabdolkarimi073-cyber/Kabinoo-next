...
export default function Page(props: any) {
    ...
    const { data: categories = [], loading: l4 } = useBackend<any[]>("/shop-manager/product/category");
    const { data: colors = [], loading: l3 } = useBackend<any[]>("/admin/product/colors");
    const { data: materials = [], loading: l2 } = useBackend<any[]>("/admin/product/materials");
    const { data: details = [], loading: l1 } = useBackend<any[]>("/admin/product/details");
    const { data, loading: l5 = false } = !isN ? useBackend<FullProduct>(`/shop-manager/product/${id}`) : {};
    ...
    if (l5 || l1 || l2 || l3 || l4) return <Loading />;
        ...
        <div className='flex flex-col gap-3 my-10 relative'>
            ...
            <div className='flex flex-wrap gap-3'>
                {product?.images && product.images.map((image, index) => image === "loading" ? (
                    <Skeleton key={`skeleton-${index}`} width={128} height={128} />
                ) : (
                    <div key={`image-${index}`} className='relative!'>
                        ...
                        <img src={image} alt={product.name} className='w-32 h-32 rounded-xl object-cover' />
                    </div>
                ))}
            ...
            <div className='center'>
                <Button loading={loading} size='lg' onClick={() => {
                    setLoading(true);
                    backend("/shop-manager/product", isN ? "POST" : "PUT", product).then(e => {
                        setLoading(false);
                        if (!e.ok) return;
                        if (isN) {
