...
export default function Page({ params }: any) {
    ...
    const { data: contract, loading, refetch } = useBackend<any>(`/contract-manager/contract/${id}?_include=user`);
    ...
    const handleSave = async () => {
        ...
        const res = await backend(`/contract-manager/contract/${id}`, "PUT", data);
        setSaving(false);
        if (res.ok) {
            setEditing(false);
            refetch();
    ...
    const handleUploadDesign = async () => {
        ...
        const res = await backend(`/contract-manager/contract/upload-design`, "POST", formData);
        setSaving(false);
        if (res.ok) {
            setDesignFile(null);
            refetch();
